...
def bbhist(x):
	edges = bayesian_blocks(x)
	los = edges[:-1]
	his = edges[1:]
	values = np.array([np.logical_and(x >= lo, x < hi).sum() / (hi - lo) for lo, hi  in zip(edges[:-1], edges[1:])])
	return edges, values
...
if __name__ == '__main__':
	...
	edges, values = bbhist(data)
	np.savetxt(filename + 'bblocks.txt', zip(edges[:-1], edges[1:], values))
	import matplotlib.pyplot as plt
	plt.bar(left=edges[:-1], width=edges[1:] - edges[:-1], height=values)
	plt.savefig(filename + 'bblocks.pdf', bbox_inches='tight')
	plt.close()
